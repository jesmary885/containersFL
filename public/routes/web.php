...
Route::middleware('auth')->group(function () {
    ...
    */
    ...
    /* ---------------------------------------------------------------
     | CONTENEDORES
     |
     | Mismo cuidado con el ORDEN que en clientes: /nuevo va ANTES que
     | /{container}. Laravel toma la primera ruta que coincida, y
     | `{container}` acepta cualquier cosa, incluida la palabra "nuevo".
     | Al reves, abrir "Nuevo contenedor" buscaria una unidad llamada
     | "nuevo" y devolveria un 404 que no senala a ninguna parte.
     * ------------------------------------------------------------ */
    Route::middleware('can:containers.view')->group(function () {

        Route::get('/operaciones/contenedores', ContainerIndex::class)
            ->name('operaciones.contenedores.index');

        Route::get('/operaciones/contenedores/nuevo', ContainerForm::class)
            ->middleware('can:containers.create')
            ->name('operaciones.contenedores.create');

        Route::get('/operaciones/contenedores/{container}/editar', ContainerForm::class)
            ->middleware('can:containers.update')
            ->name('operaciones.contenedores.edit');

        Route::get('/operaciones/contenedores/{container}', ContainerShow::class)
