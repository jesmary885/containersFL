...
class Form extends Component
{
    ...
    /*
     | El ojito: abre la ficha del cliente encima del documento, de solo
     | lectura, sin perder lo que se lleva escrito.
     */
    use \App\Livewire\Concerns\MuestraFichaDelCliente;
    ...
    /**
     * Cuándo se envió, para poder decirlo en pantalla.
     *
     * Antes el sistema sabía que estaba enviada —la columna sent_at se
     * llenaba— pero no lo decía en ninguna parte. El botón seguía
     * ofreciendo "enviar" como si no hubiera pasado nada.
     */
    public ?string $enviadaEl = null;
    ...
    protected function cargarDesde(Invoice $invoice): void
    {
        ...
        $this->enviadaEl = $invoice->sent_at?->format('d/m/Y H:i');
    ...
    public function guardar(bool $yEnviar = false)
    {
        ...
        $factura = DB::transaction(function () use ($empresa, $yEnviar) {
            ...
            if ($yEnviar) {
                ...
                /*
                 | Queda constancia en la pantalla de que ya salió, con su
                 | fecha. Es lo que apaga el botón de enviar y lo convierte
                 | en "Reenviar".
                 */
                $this->yaEnviada  = true;
                $this->enviadaEl  = $factura->fresh()->sent_at?->format('d/m/Y H:i');
            }
