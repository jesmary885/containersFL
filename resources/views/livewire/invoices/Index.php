...
class Index extends Component
{
     ...
     */
    /*
     | updatedBuscar() y updatedEstado() estaban aquí, de una línea, y más
     | abajo se agregaron otras dos con el mismo nombre. PHP no permite
     | declarar dos veces el mismo método y la aplicación no arrancaba:
     |
     |     Cannot redeclare App\Livewire\Invoices\Index::updatedBuscar()
     |
     | Se quedan las de abajo, que hacen lo mismo que estas MÁS apagar el
     | filtro de "solo con saldo". Fue un descuido al agregarlas: no
     | comprobé si ya existían.
     */
     ...
     */
    /* =====================================================================
     | BUSCAR Y FILTRAR POR ESTADO APAGAN EL "SOLO CON SALDO"
     |
     | ── EL PROBLEMA ──
     |
     | Este listado abre con la casilla "solo con saldo" MARCADA. Es un
     | buen valor de arranque: lo primero que uno quiere ver al entrar es
     | lo que hay que cobrar.
     |
     | Pero esa casilla y el desplegable de Estado son dos filtros que se
     | suman. Y eso producía dos situaciones que parecen un error del
     | sistema:
     |
     |   1. Se registra el pago de una factura, queda en Pagada, y
     |      DESAPARECE del listado. Está ahí, pero "solo con saldo" la
     |      esconde.
     |
     |   2. Se escribe el nombre del cliente en el buscador y no sale
     |      nada. Esta es la peor: buscar significa "encuéntrame esto", y
     |      el sistema contestaba con una lista vacía sin decir por qué.
     |
     |   3. Se elige "Pagada" en el desplegable de Estado y salen cero
     |      resultados SIEMPRE, porque pagada y con saldo se excluyen. Un
     |      filtro que no puede devolver nada nunca.
     |
     | ── LA REGLA ──
     |
     | Si la persona escribe algo o elige un estado, está pidiendo algo
     | concreto. Eso manda sobre el valor de arranque.
     * ================================================================== */
