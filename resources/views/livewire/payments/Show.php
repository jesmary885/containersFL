...
class Show extends Component
{
    ...
    public function mount(Payment $payment): void
    {
        ...
        $this->payment = $payment->load([
            ...
            /*
             | El papel firmado viene con la autorización. Se carga aquí y
             | no al pulsar el botón para no hacer una consulta extra por
             | cada visita a la ficha.
             */
            'cardAuth.signatureDocument',
            'createdBy',
            'allocations.invoice',
    ...
    /**
     * Descarga el formulario de autorización firmado de este cobro.
     *
     * ── POR QUÉ SE DESCARGA DESDE AQUÍ ──
     *
     * La autorización es de LA OPERACIÓN, no del cliente: autoriza un
     * cargo concreto por un importe concreto. El sitio donde alguien la
     * va a buscar es el pago que la usó, normalmente porque el banco
     * pidió el respaldo de ese cargo.
     *
     * Se comprueba que el archivo siga en el disco antes de mandarlo.
     * Sin eso, un archivo borrado a mano daba un error de servidor en
     * vez de un mensaje que se entiende.
     */
    public function descargarAutorizacion()
    {
        $this->exigirPermiso('view');

        $documento = $this->payment->cardAuth?->signatureDocument;

        if (! $documento) {
            session()->flash('error',
                'Este cobro no tiene el formulario de autorización adjunto.');
        ...
        if (! \Illuminate\Support\Facades\Storage::disk($disco)->exists($documento->path)) {
            session()->flash('error',
        ...
        return \Illuminate\Support\Facades\Storage::disk($disco)
            ->download($documento->path, $documento->name);
    }
