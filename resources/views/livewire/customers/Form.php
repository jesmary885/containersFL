...
class Form extends Component
{
    ...
    public function render()
    {
        ...
        return view('livewire.customers.form', [
             ...
             */
            /* -----------------------------------------------------------------
             | EL ARCHIVADOR DEL CLIENTE
             |
             | ── POR QUÉ SE FUE "AUTORIZACIÓN DE TARJETA" ──
             |
             | Porque no es un documento del cliente: es de LA OPERACIÓN.
             | El papel que firma autoriza un cargo concreto por un importe
             | concreto, y se agota al usarse (textual del levantamiento:
             | "autorizándome a recorrer esa tarjeta por el monto que
             | nosotros le enviamos").
             |
             | Archivarla suelta aquí la dejaba sin importe, sin fecha de
             | cargo y sin saber a qué venta correspondía. Y creaba algo
             | peor: alguien ve el archivo en la ficha, piensa "este
             | cliente ya tiene autorización" y pasa una tarjeta amparado
             | en un papel que autorizaba otro cobro de hace tres meses.
             | Eso no protege ante un chargeback; da falsa tranquilidad.
             |
             | Su sitio es el registro credit_card_authorizations, que ya
             | tiene la columna signature_document_id esperando ese
             | archivo, y se sube desde la pantalla del pago, que es donde
             | se conoce el monto. Desde ahí se descarga.
             |
             | Los documentos ya subidos con esa categoría NO se pierden:
             | siguen guardados y se siguen viendo con su etiqueta. Lo que
             | cambia es que no se pueden crear nuevos por esta vía.
             * -------------------------------------------------------------- */
            'categorias' => [
                DocumentCategory::Contract->value          => 'Contrato',
                DocumentCategory::TaxExemption->value      => 'Certificado de exención',
                /*
                 | ── POR QUÉ SE FUE "CERTIFICADO DE EXPORTACIÓN" ──
                 |
                 | Porque es del CONTENEDOR y de la venta, no del cliente.
                 | Lo emite un inspector sobre esa unidad concreta.
                 |
                 | Textual del levantamiento, Denisse:
                 |   "el de exportación lleva un certificado de exportación
                 |    por un inspector"
                 |   "que cada certificado de exportación esté con la venta,
                 |    el contenedor y el invoice. Que todo esté linkado"
                 |
                 | Tres enlaces: venta, contenedor e invoice. El cliente no
                 | está en esa lista. Archivado suelto aquí no cumplía
                 | ninguno de los tres.
                 |
                 | Su sitio es la factura, que ya tiene su zona de adjuntos.
                 | Desde ahí el contenedor lo encuentra, porque el renglón
                 | sabe qué unidad se vendió.
                 |
                 | Los ya subidos NO se pierden: siguen guardados y visibles
                 | con su etiqueta.
                 */
